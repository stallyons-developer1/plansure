...
doc.text(
  "The Governance Dashboard calculates two key values for each activity:",
  50,
  y,
);
y += 20;
...
y += 15;
doc.text(
  "    2. Status - Whether the activity needs attention (Ready, At Risk, Blocked, Complete)",
  50,
  y,
);
y += 35;
...
doc.fontSize(10).fillColor(BLACK).font("Helvetica");
doc.text(
  "RAG Zone is calculated based on the number of weeks between today's date and the activity's start date.",
  50,
  y,
);
y += 25;
...
const ragRows = [
  ...
  [
    "Activity started AND finish date NOT passed",
    "In Progress",
    "Green",
    GREEN,
  ],
  [
    "Start date within 1-2 weeks (1-14 days)",
    "Week 1 or Week 2",
    "Green",
    GREEN,
  ],
  [
    "Start date within 3-4 weeks (15-28 days)",
    "Week 3 or Week 4",
    "Amber",
    AMBER,
  ],
  ...
  [
    "Start date beyond 6 weeks (43+ days)",
    "X Weeks (e.g., 20 Weeks)",
    "Grey",
    GREY,
  ],
];
...
doc.fontSize(10).fillColor(BLACK).font("Helvetica");
doc.text(
  "Status is determined by checking multiple conditions in priority order:",
  50,
  y,
);
y += 25;
...
const meanings = [
  {
    status: "Ready",
    color: GREEN,
    meaning: "Activity is on track, no issues. No immediate action required.",
  },
  {
    status: "At Risk",
    color: AMBER,
    meaning: "Activity needs attention. Review and resolve issues.",
  },
  {
    status: "Blocked",
    color: RED,
    meaning: "Activity cannot proceed. Remove blocker immediately.",
  },
  {
    status: "Complete",
    color: GREEN,
    meaning: "Activity is finished. No action required.",
  },
];
...
doc
  .moveTo(150, timelineY - 15)
  .lineTo(150, timelineY + 15)
  .stroke();
doc
  .fontSize(8)
  .fillColor(BLACK)
  .text("TODAY", 135, timelineY - 25);
